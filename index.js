...
function processSum(numberList, callback) {
  const allElements = numberList.length;
  let sum = numberList.reduce((acc, curentValue) => {
    return acc += curentValue.sum;
  }, 0)
}
...
function getFullNames(runners) {
  ...
  runners.forEach((runner) => {
    fulNames.push(`${runner.last_name}, ${runner.first_name}`);
  });
  return fulNames;
}
...
function firstNamesAllCaps(runners) {
  return newRunngiters = runners.map((runner) => {
...
*/
function getRunnersByTShirtSize(runners, tShurtSize) {
  return runners.filter((runner) => {
    return tShurtSize === runner.shirt_size;
  });
}
